...
with fitz.open(stream=file_bytes, filetype="pdf") as doc:
        ...
        if v_mode == "네이티브 뷰어 (드래그 가능)":
            ...
            with st.expander("📝 현재 페이지 정밀 텍스트 추출", expanded=True):
                ...
                try:
                    ...
                    # 백업: 추출 결과가 비어있으면 단순 텍스트 추출
                    if not result_text.strip():
                        result_text = page.get_text("text", sort=True)

                except Exception as e:
                    st.warning(f"구조화 추출 실패, 단순 추출로 전환합니다: {e}")
                    result_text = page.get_text("text", sort=True)

                # ── 출력 ──
                st.markdown(result_text)
                st.text_area(
                    "✂️ 드래그 복사용 (아래에서 선택 후 복사)",
                    value=result_text,
                    height=350,
                )
    ...
    with col_tool:
        ...
        raw_input = st.text_area(
            "분석할 문단을 여기에 붙여넣으세요",
            height=200,
            placeholder="왼쪽에서 긁은 내용을 붙여넣으세요.",
        )
                ...
                with st.spinner("전문 용어 최적화 직역 중..."):
                    try:
                        res = model.generate_content(
                            f"당신은 생체역학 전공 번역가입니다. 다음을 한국어로 정확히 직역하세요:\n\n{raw_input}"
                        )
                        st.info(f"**[직역 결과]**\n\n{res.text}")
                    except Exception as e:
                        st.error(f"분석 오류: {e}")
        ...
        if c2.button("🧠 심층 역학 분석"):
            if raw_input.strip():
                with st.spinner("생체역학적 기전 분석 중..."):
                    try:
                        res = model.generate_content(
                            f"당신은 생체역학 박사입니다. Kinetics/Kinematics 관점에서 상세 분석하세요:\n\n{raw_input}"
                        )
            ...
            if chat_query or data_img:
                ...
                with st.spinner("AI 분석 중..."):
                    try:
                        ...
                        if data_img:
                            prompt.append(Image.open(data_img))
