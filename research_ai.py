...
if pwd:
    stored = st.secrets.get("LAB_PASSWORD_HASH", hash_password("1234"))
    # [수정] 평문 비교 대신 해시 비교
...
# 3. Gemini 모델 연결 (수정: fallback 우선순위 + 안전한 예외처리)
...
with fitz.open(stream=file_bytes, filetype="pdf") as doc:
    ...
    with col_pdf:
        ...
        # [수정] 대용량 PDF의 경우 base64 전체 임베딩 대신 현재 페이지만 이미지로 렌더링
        # 새 창용 링크는 유지하되 경고 추가
        ...
        if file_size_mb < 10:
            ...
            st.warning(f"⚠️ 파일 크기({file_size_mb:.1f}MB)가 커서 새 창 보기는 비활성화됩니다. 이미지 모드를 사용하세요.")
            pdf_data_url = None
        ...
        if v_mode == "원본 이미지 모드":
            ...
            if pdf_data_url:
                st.markdown(
                    f'<iframe src="{pdf_data_url}" width="100%" height="800"></iframe>',
                    unsafe_allow_html=True,
                )
            else:
                st.error("파일이 너무 커서 인터랙티브 모드를 사용할 수 없습니다. 이미지 모드를 선택하세요.")

        # ── 텍스트 추출 ─────────────────────────────
        st.markdown("---")
        with st.expander("📋 논문 텍스트 정밀 추출 (가독성 최적화)", expanded=True):
            try:
                # [수정] "lines" 키 존재 여부 명시적 확인으로 KeyError 방지
                blocks = page.get_text("dict", flags=11)["blocks"]
                structured_text = ""

                for b in blocks:
                    if b.get("type") != 0:   # 0 = 텍스트 블록, 나머지(이미지 등) 스킵
                        continue
                    lines = b.get("lines", [])
                    if not lines:
                        continue
    ...
    with col_tool:
                ...
                with st.spinner("전문 번역 중..."):
                    try:
                        res = model.generate_content(
                            f"스포츠 생체역학 전문가로서 다음 내용을 한국어로 자연스럽게 번역하세요. "
                            f"전문 용어는 최대한 유지하세요:\n\n{raw_input}"
                        )
                        st.info(res.text)
                    except Exception as e:
        ...
        if c2.button("🧠 심층 역학 분석"):
            if raw_input.strip():
                with st.spinner("역학적 기전 분석 중..."):
                    try:
                        res = model.generate_content(
                            f"생체역학 박사로서 다음 연구 내용의 Kinetics 및 Kinematics적 의미를 분석하세요:\n\n{raw_input}"
                        )
                        st.success(res.text)
                    except Exception as e:
                        st.error(f"분석 오류: {e}")
        ...
        # [수정] JavaScript 붙여넣기는 Python AI 전송 브릿지 없음을 명시하고
        #        file_uploader 방식만 AI 전송에 사용
        st.caption(
            "⚠️ Ctrl+V 미리보기는 화면 확인용입니다. "
            "AI 분석은 아래 '사진첩에서 선택'으로 업로드한 이미지만 전송됩니다."
        )

        paste_html = """
        <div id="p-area" style="border:2px dashed #4CAF50; padding:15px; text-align:center;
            cursor:pointer; border-radius:10px; background-color:#f9f9f9;">
            여기 클릭 후 <b>Ctrl+V</b>로 그래프 미리보기 (화면 확인용)
        </div>
        ...
        if data_img:
            st.image(data_img, width=300)

        chat_query = st.text_area("질문을 입력하세요", height=100)

        if st.button("🚀 분석 전송"):
            if not chat_query.strip() and not data_img:
                st.warning("질문 또는 이미지를 입력하세요.")
            else:
                # [수정] 채팅 히스토리를 append로 시간순 유지
                ...
                with st.spinner("AI 분석 중..."):
                    try:
                        ...
                        if data_img:
                            content_parts.append(Image.open(data_img))
