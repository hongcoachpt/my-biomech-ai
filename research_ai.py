...
# 🚩🚩🚩 [수정 확인 포인트] 🚩🚩🚩
        # 파이썬(Streamlit)은 업로드 창이 2개 이상이 되면 "어디로 붙여넣어야 하지?" 하고
        # 충돌을 일으켜서 붙여넣기를 차단해 버립니다.
        # 그래서 가장 안정적인 '단일 박스'로 통합하고 안내 문구를 직관적으로 바꿨습니다.
        
        st.info("💡 **마법의 캡처 붙여넣기 비법:**\n"
                "1. 버튼을 누르지 마세요! (탐색기 창이 뜹니다)\n"
                "2. 마우스로 **앱 화면의 빈 곳(흰 여백)을 툭 한 번 클릭**하세요.\n"
                "3. 그 상태로 **Ctrl+V**를 누르시면 아래 상자로 이미지가 쏙 들어갑니다!")
        
        data_img = st.file_uploader("📂 (PC에서 직접 파일을 찾을 때만 이 네모 박스를 클릭하세요)", type=["png", "jpg", "jpeg"])
